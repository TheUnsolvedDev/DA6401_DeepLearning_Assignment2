...
def test_model():
    ...
    with wandb.init(project='DA24D402_DL_2') as run:
        fig, ax = plt.subplots(10, 10, figsize=(60, 18))
        plt.tight_layout()
        for x in range(10):
            for y in range(3):
                ...
                if classes[batch_labels[x*3+y]] == classes[batch_predictions[x*3+y]]:
                    ...
                    color = 'red'
                ax[y][x].set_title(
                    f'True label: {classes[batch_labels[x*3+y]]} \n Predicted: {classes[batch_predictions[x*3+y]]}', color=color)
        wandb.log({"plot": fig})
