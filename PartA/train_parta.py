...
def model_parser(num_filters_for_each_layer: int, activation_function: str, filter_organisation: str, data_augmentation: bool, batch_normalisation: bool, dropout_rate: float, filter_size: int, dense_number: int, learning_rate: float = 1e-4):
    """
    ...
    model = CNN(conv_filters=filters, dropout_rate=dropout_rate, activation=activation, batch_normalization=batch_normalisation,
                conv_kernel_sizes=conv_filters_sizes, conv_strides=conv_filter_strides, dense_units=dense_number, learning_rate=learning_rate).to(device)
...
def main():
    ...
    parser.add_argument("-wp", "--wandb_project", default="DA24D402_DL_2",
    ...
    parser.add_argument('-act', '--activation', default='ReLU', type=str,
                        choices=['ReLU', 'GeLU', 'SiLU', 'Mish', 'LeakyReLU', 'Tanh', 'Sigmoid'], help='Choice of activation function')
